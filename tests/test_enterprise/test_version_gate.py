...
class TestPhaseNone:
    def test_no_flag_accepts_unsigned_peer(self, monkeypatch):
    ...
    def test_no_flag_phase_is_none(self, monkeypatch):
        monkeypatch.delenv("SC_SUNSET_V1", raising=False)
        assert VersionGate.current_phase(now=NOW) == "none"


# ── Phase 1: grace period ──────────────────────────────────────────────────────

class TestGracePeriod:
    def test_unsigned_peer_accepted_in_grace(self, monkeypatch):
    ...
    def test_signed_valid_peer_accepted_in_grace(self, monkeypatch, tmp_path):
        ...
        with patch("enterprise.registry.get_agent_prop", side_effect=store.get), \
            ...
            result = gate.check_peer(
                FAKE_HWND, identity.public_key_bytes, now=NOW, max_sig_age_seconds=0.0
            )
...
class TestEmergencyOverride:
    def test_override_accepts_all_peers(self, monkeypatch):
        ...
        assert result.ok is True
        assert result.phase == "override"

    def test_override_phase_returns_override(self, monkeypatch):
        monkeypatch.setenv("SC_DISABLE_SIG_VERIFY", "1")
        assert VersionGate.current_phase(now=NOW) == "override"


# ── Bad SC_SUNSET_V1 format ───────────────────────────────────────────────────

class TestBadSunsetFormat:
    def test_invalid_date_disables_enforcement(self, monkeypatch):
        ...
        # Invalid date → _sunset_date() returns None → Phase 0 (no enforcement)
        assert result.ok is True
        assert result.phase == "none"
