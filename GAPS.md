...
| CC-19 | Durable approval state could change without an independently chained transition event | Hardened `DurableOperatorQueue` now stages request, approve, deny, consume, and expiry in a same-database outbox, keeps the capability non-authorizing until the signed-ledger receipt is durable, reconciles interrupted appends idempotently, and makes MCP actuation recheck the consumed binding and ledger chain. The selected deployment must still supply and assess its decision-writer credential verifier. | Fixed in code + adversarial restart/tamper tests; deployment credential format remains environment-specific | **CLOSED IN CODE 2026-07-17; deployment identity assessment open** |
