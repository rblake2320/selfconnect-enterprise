...
| CC-19 | Durable approval state could change without an independently chained transition event | Hardened `DurableOperatorQueue` now stages request, approve, deny, consume, and expiry in a same-database outbox, keeps the capability non-authorizing until a fully verified signed-ledger receipt is durable, repeats state validation under a write transaction, retains a bounded nonce-bound decision-proof envelope, and requires an exact request/approval/consumption lineage before MCP actuation. Ledger append failures restore the prior verified tail. The selected deployment must still supply and assess its decision-writer credential verifier. | Fixed in code + adversarial disk/restart/TOCTOU/forgery/schema tests; deployment credential format remains environment-specific | **CLOSED IN CODE 2026-07-17; deployment identity assessment open** |
