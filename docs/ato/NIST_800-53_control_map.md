...
| AC-6 | Least Privilege | Agent identities use standard user DPAPI scope (`CryptProtectData` without `CRYPTPROTECT_LOCAL_MACHINE`). The target guard uses `PROCESS_QUERY_LIMITED_INFORMATION` (0x1000) to read the owning executable path. Named pipe DACL and impersonation behavior remain deployment-dependent controls requiring live verification. | `enterprise/identity.py:_dpapi_encrypt()`, `experiments/win32_probe/target_guard.py:_exe_path()`, `experiments/win32_probe/chained_channel.py:role_b()` | Implemented components; deployment verification required |
