...
| 10 | WR-001 fix: pre-registered expected_pub | Red-team finding + fix | `enterprise_experiments/win32_probe/chained_channel.py:role_b()` | Named pipe server pre-pins expected TPM public key blob; self-signed substitution attack rejected | IA-3, SC-8, CA-2 | 2026-06 |
| 11 | WR-003 fix: random pipe name + `FILE_FLAG_FIRST_PIPE_INSTANCE` | Red-team finding + fix | `enterprise_experiments/win32_probe/chained_channel.py:_unique_pipe_name()`, `role_b()` | Pipe name prediction race mitigated via 128-bit entropy + OS-enforced first-instance flag | SC-4, AC-4 | 2026-06 |
| 12 | WR-005 experiment: `SW_HIDE` on throwaway consoles | Red-team finding + mitigation | `enterprise_experiments/win32_probe/chained_channel.py:main()` | The throwaway console is launched without a visible window. `SW_HIDE` is not a security boundary and does not prove it is unavailable to same-session UI Automation | SC-4, AU-9 | 2026-06 |
| 13 | WR-009 fix: challenge-response nonce | Red-team finding + fix | `enterprise_experiments/win32_probe/chained_channel.py:role_b()` (challenge_nonce), `main()` | Server-generated nonce folded into `SHA-256(delta + nonce)`; replay of pre-captured payloads rejected | SC-8, IA-3 | 2026-06 |
...
2. Exit code 0 from `python enterprise_experiments/win32_probe/chained_channel.py` — records that the exact local experimental chain completed in that environment.
...
4. Hash of `enterprise/identity_gate.py`, `enterprise/identity.py`, `enterprise_experiments/win32_probe/target_guard.py`, and `enterprise_experiments/win32_probe/chained_channel.py` — to detect post-deployment modification.
